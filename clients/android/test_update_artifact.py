...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='paranoid-update-payload-') as d:
        ...
        run('artifact-java',['javac','--release','8','-Xlint:-options','-cp',ROOT/'out/update-host','-d',tmp,ROOT/'test/UpdateArtifactSmoke.java'])
        run('actual-artifact-policy',['java','-cp',str(tmp)+':'+str(ROOT/'out/update-host'),'org.paranoid.text.UpdateArtifactSmoke',metadata_file,apk,current['package'],str(current['version_code']),str(current['min_sdk']),current['cert'],old['package'],str(old['version_code']),old['cert']])
        ...
        print(json.dumps(dict(apk=str(apk),sha256=current['apk_sha256'],size=current['apk_size'],version=6,signer=result['signer_sha256'],fresh_payload_match=True,checks=len(records)),indent=2))
