...
public final class UpdatePolicySmoke {
    ...
    public static void main(String[] args)throws Exception {
        ...
        for(String bad:new String[]{j+"{}",j.replace("\"schema\":1","\"schema\":1,\"schema\":1"),j.replace("\"schema\":1","\"schema\":1,\"x\":1"),j.replace("\"schema\":1","\"schema\":1.0"),j.replace("\"schema\":1","\"schema\":\"1\""),j.replace("\"schema\":1","'schema':1"),j.replace(":6",":06"),j.replace(":6",":-1"),j.replace(":6",":9223372036854775808"),j.replace("global.paranoid.messenger","other.package"),j.replace("arm64-v8a","x86"),j.replace("\"apk_size\":3","\"apk_size\":9223372036854775808"),j.replace("\"apk_size\":3","\"apk_size\":0"),j.replace(UpdateSmoke.repeat('a',64),"../escape"),j.replace("0.0.6-update",UpdateSmoke.repeat('x',129)),j.replace("0.0.6-update","bad\\u0000name"),j.replace("0.0.6-update","bad\\ud800name"),j.replace("}",",}")})reject(()->UpdateManifest.parse(bad.getBytes(StandardCharsets.UTF_8)));
