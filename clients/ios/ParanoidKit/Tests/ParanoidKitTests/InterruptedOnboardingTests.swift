...
/// Test-only Sendable value boundary: retain full credential equality without
/// retaining a Foundation object graph derived from an actor-bound client.
private func credentialBytes(_ client: SelfServiceClient) throws -> Data {
    try JSONSerialization.data(withJSONObject: client.credential(), options: [.sortedKeys])
}

private func decodeCredential(_ data: Data) throws -> [String: Any] {
...
final class InterruptedOnboardingTests: XCTestCase {
    ...
    func testSchemaZeroCheckpointResumesSameIdentityBeforeRegistration() async throws {
        ...
        let credential = try credentialBytes(resumed)
        XCTAssertTrue(try resumed.hasIdentity())
        XCTAssertEqual(try device.stored(), checkpoint, "opening only validates; no write")
        let transport = FakeTransport(server: try StandServer(credential: decodeCredential(credentialBytes(resumed))))
        transport.answer = { call in
            if call.path == ChallengeIntent.registrationChallengePath {
                ...
                XCTAssertEqual(try credentialBytes(reopened), credential)
            }
            return nil
        }
        ...
        XCTAssertEqual(try credentialBytes(final), credential)
    ...
    func testActiveCheckpointPreparesDurableQRWithoutRegisteringAgain() async throws {
        ...
        let credential = try credentialBytes(device.client)
        failSecondWrite(on: device)
        ...
        transport.answer = { _ in
            ...
            XCTAssertEqual(try credentialBytes(durable), credential)
            return nil
        }
        let owner = StateOwner(client: resumed)
        let flow = ProofFlow(owner: owner, transport: transport, pacer: RecordingPacer())
        ...
        XCTAssertEqual(try peer.client.previewContact(contact)["account"] as? String,
                       try decodeCredential(credential)["account"] as? String)
