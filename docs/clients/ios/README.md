...
- Proposed, awaiting the owner: new-message counts and the «Новые сообщения»
  divider exist on iOS only, and in memory only for the current run
  ([text and the lanes](self-service.md#text-receipts-and-the-lanes)). Android
  has no such markers yet.
