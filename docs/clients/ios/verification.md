...
Two hosted accounts exist (`hosted_registrations` is 2: one `service-bridge`
registration from the build Mac to isolate the phone's TLS failure, one from
the physical iPhone after the App Transport Security fix; both under the
owner's answer to RFC-0021 question 4, no fixed budget; the session of
2026-09-14 consumed none, it used the account registered on 2026-09-13).
Before those, the only contact this branch had had with the hosted server was
one TLS handshake that compared the live SubjectPublicKeyInfo digest against
the pin this client carries.
...
iPhone registered on 2026-09-13, and `hosted_registrations` is still 2. Stage 1
also had a solo pre-run on 2026-09-13 (steps 1, 2, 4 and the first half of 5,
by the contributor alone with the owner's QR image), which is not the joint
test. Every other step of both stages is still `NOT RUN`:
