...
## Package/simulator receipt for the c735f94 main checkpoint (4066f36)
...
results are quoted in the coordinator's bot-authored merge-gate review. PR50
merged as `02baeb2`.
`git diff 4066f36 c735f94 -- clients/ios clients/core key-protocol` is empty,
so the receipt is relevant to those sources at that exact main checkpoint,
not to arbitrary later heads or bit-identical build artifacts. PR46's earlier
...
| New-message marks («Новые сообщения», in memory only): a row of «Чаты» counts the peer's messages this run has not shown; the chat opens at the divider; «↓» leads down; a message arriving at the open bottom is seen at once; a reader scrolled up is not dragged; nothing is stored or sent (REQ-MSG-003 unchanged) | Yaroslav's Mac, 2026-09-26: `SeenMarksTests` 10/0 (all RED on a stub first), `UnreadTimelineTests` 7/0, `test_ui_contract.py` new-marks check (RED on `origin/main`), and the stand-backed `test_sim_text.py` text flow with the new steps 16-19a (twelve new messages over more than a screen, «↓», a message arriving while scrolled up); mutation checks turned RED. See [receipt](../../project/evidence/ios-client-20260926/unread-markers-mac.md). No physical phone; the iOS 17 approximation not run; VoiceOver not spoken | CLAIMED |
