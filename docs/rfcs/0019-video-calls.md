...
- `v` becomes 2 for clients that support video. A v2 client sending to a peer
  that only understands v1 sends `v: 1` audio-only bodies; the capability is
  learned from the peer's `ready` (v2 `ready` carries `video: true|false`).
  A v1 peer rejects unknown fields, so a v2 body is never sent before a v2
  `ready` is received.
- New field `video` (boolean) on `knock`, `ready`, `offer`, `answer`: whether
...
- SDP limit rises from 6144 to 12288 UTF-8 bytes; the frame2 limit (16384)
  stays. Candidate limits unchanged.
...
- Threat model delta to be written in `docs/security/video-v1-threats.md`
...
- Disposition: draft, awaiting owner review.
...
- Resulting ADR: to be created as ADR-0013 after disposition.
