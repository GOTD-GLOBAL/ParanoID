...
| Relay gathering delays or republishes authenticated SDP | Successful local installation, usable component-1 relay candidate, one500 ms owner timer and immutable snapshot; direct mode still waits for COMPLETE | Actual publication RED/GREEN, both roles, cancel during coalescing, late COMPLETE with callback count1 and unchanged fingerprint/ICE, native acceptance; no deadline extension/fallback |
