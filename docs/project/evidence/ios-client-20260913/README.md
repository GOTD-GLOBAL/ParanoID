...
| [artifacts.json](artifacts.json) | the same catalogue for machines: 46 artifacts and 61 screenshots, each with path, bytes, SHA-256 and the time it was produced |
...
## PR43 receipt and final 96298cd integration review

- [dad2f7d Mac receipt](mac-receipt-pr43-dad2f7d.md): C1 7/0, package 300/0,
  lazy-key 14/0, storage 25/0, signed-simulator app 64/0; old cb52330 behavioral
  baseline seven tests/thirteen expected assertion failures, no unexpected.
- [Final 96298cd review](pr36-review-96298cd.md): tested code identity preserved
  through documentation-only conflict resolution; C1 closed at host/simulator
  scope. No main merge, architecture acceptance or device/live authority implied.
...
| Counter | Value |
| --- | --- |
| Accounts this branch created on the hosted alpha | `hosted_registrations: 3` — one from the build Mac through `service-bridge` on 2026-09-13 while diagnosing the phone's TLS failure; one from the physical iPhone on 2026-09-13 once App Transport Security was switched off, which is the contributor's own account and still in use; and one from the build Mac on 2026-09-14 at 08:26:45.596+03:00 (`POST /v2/registration/commit`, 200), a diagnostic account with no messages and no contacts, registered to read the hosted server from a second identity while diagnosing issue #38. All three under the owner's answer to RFC-0021 question 4 (no fixed budget). A third was needed because the first is dead: that fixture kept its wrapping key in process memory only, so its state file no longer opens and the account, though registered, is unreachable; the 2026-09-14 account keeps its key beside its state in git-ignored build output, so it can be read again and that diagnosis needs no further registration. The phone then paired the owner's Android from its QR and sent a text the hosted server accepted. The joint session of 2026-09-14 ran on that same iPhone account and registered nothing, so the counter is unchanged by it |
| Contacts with the hosted server | one TLS handshake from the build Mac with no HTTP request (2026-09-13T08:06Z); then the build Mac's `service-bridge` registration, made while diagnosing the phone's TLS failure; then, after the App Transport Security fix of `adb56be`, the iPhone's registration, the pairing of the owner's Android from his QR image on the iPhone, and one text the hosted server accepted (one check) — its delivery to the owner's Android was still pending because his phone had not polled; then, on 2026-09-14, the unscheduled joint session on that same account, which registered nothing: text both ways with the owner's Android and one call it placed to the iPhone; then, later that day, the build Mac's registration of the diagnostic account and the signed reads it was made for, comparing a second identity against the iPhone's while diagnosing the intermittent hosted failure of issue #38 |
...
tests need, no fixed budget** — which sets a budget, not a number. The three
registrations above, the diagnostic one of 2026-09-14 included, are recorded
under that answer with the reason each was needed, and no separate
per-registration permalink is written beside any of them.
...
| 8 | ParanoidKit tests | `swift test --package-path ParanoidKit --scratch-path out/spm` | ok | 11 | 242 tests, 0 failures in that run; re-run at this head on 2026-09-14 it prints **261 tests, 0 failures** |
...
| 21 | simulator Keychain tests | `xcodebuild test … -only-testing:ParanoIDTests/KeychainStoreTests` | ok | 6 | 8 tests, 0 failures in that run; the only run where the application owns a Keychain (ad-hoc simulator signature, no team) — measured, see below. Re-run at this head on 2026-09-14: **11 tests, 0 failures**, the three added ones being the interrupted first run, the lost marker beside a state file and the installation from a build before the commit record. Re-run later the same day on the working tree that replaced the commit record with `paranoid.firstrun.pending.v1`: **12 tests, 0 failures** (`** TEST SUCCEEDED **`), the twelfth being the lost commit record beside a lost file, which freezes with the key kept, and the file-without-key case now also holding that the freeze withdraws nothing |
...
| The same iPhone against the hosted alpha: a Release build signed with the same team at `adb56be`, after the App Transport Security fix | 2026-09-13, recorded in the same file | the phone registered (`hosted_registration_from_device: true`); it paired the owner's Android from his QR image, scanned off a screen, and sent one text the hosted server accepted (one check); delivery to the owner's Android `pending` — his phone had not polled. Before the fix, a `service-bridge` registration from the build Mac diagnosed the phone's TLS failure; the two together are two of the counter's 3, the third being the diagnostic account registered from the build Mac on 2026-09-14 | the same `device-smoke-result.json` |
| Connectivity-restart run on one disposable simulator (iPhone 17 Pro, iOS 26.5) against a disposable local stand, from a Debug build of the working tree: the recorded drop reproduced by freezing and resuming the stand's server, «Повторить подключение» pressed during the outage and again after the restore, a second tap while connected, and the shipped watcher/runner/policy chain driven at `NetworkWatcher.report(_:)`. The stand, the simulator and two temporary probe test files were removed afterwards | 2026-09-14 | the button cancels the running long poll at `[-999]` and the client is green 5.296 s after the restore against 20.002 s untouched; a tap during the outage cancels what is in flight, says «Подключение · подробнее» and invents no green state; a tap while connected takes Android's gate and only wakes the send lane; a stated change of the carrying interface reaches `RealtimeLoop.restart()` in 0.069 ms and the relaunch in 0.096 ms, while a first path, the same path again, an unsatisfied path and a change with the lanes stopped each restart nothing. **No real path change was produced** — see the row in the NOT RUN table | `reconnect-20260914/reconnect-result.json` |
| Storage and camera rules after the owner-side review of the P1 answers: `swift test --package-path ParanoidKit --scratch-path out/spm`, `xcodebuild test -only-testing:ParanoIDTests/KeychainStoreTests` on `iPhone 17 Pro (26.5)` with the signature gate 21 uses, `python3 test_ui_contract.py`, `python3 test_docs_consistency.py`, `python3 test_component_boundary.py` | 2026-09-14 | 261 tests, 0 failures on the host; 11 tests, 0 failures on the signed simulator, including the container installed before the commit record, which still freezes over a missing state file; `OK` (20 tests), `PASS: 7 facts`, `PASS: 0 outside allowlist`. No new artifact file: these are gates, and their output is the line above | none (gate output only) |
...
| The storage rule after the second owner-side review — the commit record replaced by the positive `paranoid.firstrun.pending.v1` — on the uncommitted working tree: `swift test --package-path ParanoidKit`, `xcodebuild test -only-testing:ParanoIDTests/KeychainStoreTests` on `iPhone 17 Pro (26.5)` with the signature gate 21 uses, `python3 test_ui_contract.py`, `python3 test_docs_consistency.py`; and the reviewer's scenario run unchanged against a scratch copy of `67af672`'s `Storage/` | 2026-09-14 | **266 tests, 0 failures** on the host, 27 of them `SnapshotStoreTests` (one a strict expected failure that holds the disclosed remainder); **12 tests, 0 failures** on the signed simulator (`** TEST SUCCEEDED **`), among them the lost commit record beside a lost file, which freezes with the key kept; `OK` (20 tests); `PASS: 7 facts`. The scratch copy: the same test file against `67af672`'s `Storage/` printed the facts after the commit as `paranoid.install.v1`, `paranoid.install.v2` and `paranoid.snapshot.v1`, removed `paranoid.snapshot.v1` and then the file, and the launch answered `fresh` on both attempts — `Executed 1 test, with 2 failures`, exit 1 — where the same file against the working tree's `Storage/` answers `frozen` and passes. No new artifact file: these are gates, and their output is the line above | none (gate output only) |
...
is where two of the three registrations, the pairing from the owner's QR image
and the one accepted text of the **Counters** table come from. The
`server_state` block of `device-smoke-result.json` (5 accounts, 5 devices,
7 frame-2 envelopes, 0 plaintext rows) is the **local** stand's — its `stand`
line says so — and no such count was taken on the hosted server.
...
| `clients/ios/out/evidence/reconnect-20260914/reconnect-result.json` | 2026-09-14T05:58Z | 10376 | `cc9f784309ca6bd7a733572c3a63f0dfc6afd76acd053d3056f4c71e5ffc733f` | the connectivity-restart run: the recorded unfixed timings beside the measured fixed ones, the two button presses, what the real `NWPathMonitor` reports inside a simulator, the confounds, and a `not_run` list whose first entry is the path change that never happened. The same directory holds the four timelines in the shape of the recorded reproduction (`fixed1-`, `notap-`, `restoretap-`, `button-timeline.txt`), the CFNetwork subset of each unified log, one console log, five screenshots, `path-monitor.json`, `path-chain.json` and the prose report, each named in this file's own `artifacts` list; the raw material is under `clients/ios/out/reconnect-run`, which is git-ignored. Origin, port, pin, account and the server's process id are masked |
...
way; none of them was taken on the iPhone. Five more, under
`reconnect-20260914/`, are the connectivity-restart run, listed there the same
way and taken on a simulator as well.
...
| `clients/ios/out/evidence/reconnect-20260914/` | 5 | the connectivity-restart run on one disposable simulator: the client registered on the stand, «Сервер подключён» after the restore with no tap, «Сервер подключён» 1.1 s after the tap that followed the restore, and the connection sheet on each of the two taps — «Подключение · подробнее» during the outage, «Подключено» for the tap that was gated while connected |
...
| What became of the one hosted text of 2026-09-13 | It was accepted by the hosted server (one check) and the owner's phone had not polled. The next day's session exchanged text both ways with both checks on the iPhone, but the report does not say whether that earlier envelope was among what his phone finally polled. The three registrations are counted above; the server cannot delete an account |
...
| A real change of network path, on any device — the open item of 2026-09-13/14 | That open item has a cause and a fix on this branch: after a network drop the application stayed at «Нет подключения» while the server answered from the Mac and the pinned key was unchanged, because this client had no connectivity-change restart — the defect Android had reported to it on 2026-09-12 and fixed the same day (`TextEngine.java:200-215`, `RealtimeLoop.java:59-67`, read in this branch's merged Android tree `0.0.22-push`, not at `fe9c26c`). The fix and its run are the rows above and are `CLAIMED`, on a simulator against the local stand. **No real path change was produced for it and none is claimed:** a simulator has no network of its own, so `NWPathMonitor` there reports the build Mac's default path; `xcrun simctl` has no network verb; and moving the Mac's own default route needs an administrator and would have taken down its only WAN interface. The rule was driven at the seam it exists for, through the shipped watcher, runner and policy. Nothing was run on a phone, nothing against the hosted alpha, and no device log was ever collected for the original drop; the drop that was reproduced freezes the server, which produces a timeout and not a path change, so the 25 s of the recorded reproduction against the 20 s measured here is where the backoff landed and is not an improvement. The console read of 2026-09-14 above is still the other failure, and this fix does not address it |
...
contributor's own iPhone and two of the three hosted registrations happened on
2026-09-13, and the third — the diagnostic account — on 2026-09-14. All three
are recorded in the **Counters** table under the question-4 answer, with no
separate permalink beside them.
...
3. For stage 1, a hosted registration for each iPhone identity the test needs.
   ...
   before the stage: the counter reads 3 in total, 1 for the phone.
